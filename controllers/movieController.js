...
exports.getMovieById = async (req, res) => {
  try {
    ...
    console.log("Movie ID:", id, "User ID:", userId);
...
exports.postFav = async (req, res) => {
  try {
    ...
    // Step 2: Check if the movieId is already in the user's favourites array
    if (user.favourite.includes(movieId)) {
      return res.status(400).json({ message: 'Movie is already in your favourites' });
    }

    // Step 3: Add the movieId to the user's favourite array
    user.favourite.push(movieId);

    // Step 4: Save the user with the updated favourites
    await user.save();
...
exports.uploadAuthor = async (req, res) => {
  try {
    ...
    console.log(name,dob,bio,nationality)
